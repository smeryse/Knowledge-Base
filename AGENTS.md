...
- Current entrypoints are thin wrappers:
  - `LifeOS/Кухня/Templates/Скан товара.md` -> `tp.user.food_scan(tp)`
  - `LifeOS/Кухня/Templates/Новый чек.md` -> `tp.user.food_db(tp)`
  - `LifeOS/Кухня/Templates/Проверка LLM.md` -> `tp.user.food_llm_check(tp)`
  - `LifeOS/Кухня/Templates/OCR чек.md` -> `tp.user.food_receipt_ocr(tp)`
...
- After changing the food schema or prompts, also verify the paired Markdown templates/docs stay aligned. The important files are:
  - `LifeOS/Кухня/Templates/*.md`
  - `LifeOS/Кухня/Docs/Schema.md`
  - `LifeOS/Кухня/Docs/README.md`

## Food Project
- `LifeOS/Кухня/Кухня.md` is the navigation hub for the food database; `LifeOS/Кухня/Docs/Schema.md` is the architecture reference.
- The barcode/LLM resolver config is `LifeOS/Кухня/resolver-config.json`. Verified defaults are `provider: lmstudio`, endpoint `http://127.0.0.1:1234/v1`, model `qwen2.5-3b-instruct`.
