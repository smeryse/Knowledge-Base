...
for (const field of ['top', 'bottom', 'shoes', 'socks', 'outerwear', 'accessories']) {
    ...
    for (const part of val.split('/')) {
        ...
        data.yaml.wears_count = (data.yaml.wears_count || 0) + 1;
        
        if (data.yaml.wears_count >= data.yaml.wash_after_wears) {
            data.yaml.is_dirty = true;
            data.yaml.laundry_basket = data.yaml.color_group === 'black' ? 'black' :
                                      data.yaml.color_group === 'white' ? 'white' : 'colored';
        }
        ...
        updated.push(`${data.yaml.name}: ${data.yaml.wears_count}x`);
    }
}

if (updated.length > 0) {
    new Notice(`✅ Трекнуто: ${updated.length} вещей`);
    
    // Обновляем чекбокс
    const newContent = content.replace(/- \[ \] Образ надет/, '- [x] Образ надет');
    await app.vault.modify(file, newContent);
} else {
    new Notice('⚠️ Не удалось обновить');
}
%>
